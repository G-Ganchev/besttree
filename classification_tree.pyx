...
class BinaryClassificationTree:
    ...
    def _best_split(self, X, y):
        ...
        for idx in self.feature_names.map(self.feature_map):
            ...
            num_left = np.array(num_left,dtype=float)
            num_right = np.array(num_right,dtype=float)
            ...
            best_gini,best_idx,best_split_type,best_thr = self._gini_loop(classes,
            num_left,num_right,best_gini,thresholds,best_idx,idx,split_type,best_split_type,best_thr)   
    ...
    def _gini_loop(self,long[:] classes, double[:] num_left, 
             double[:] num_right,double best_gini,
             double[:] thresholds, int best_idx,int idx,
             str split_type,str best_split_type,double best_thr):
        cdef int m = len(classes)
        cdef int i
        cdef int c
        cdef int n_clases = 2
        cdef double gini_right, gini_left
        cdef double gini = 0.0
        

        num_left = np.copy(num_left)
        num_right = np.copy(num_right)
        ...
        for i in range(1,m):
            ...
            gini_left = 1.0 - sum(
                        pow(num_left[x] / i,2) for x in range(n_clases)
                    )
            gini_right = 1.0 - sum(
                        pow(num_right[x] / (m - i), 2) for x in range(n_clases)
                    )
            ...
            if gini < best_gini:
                ...
                best_thr = (thresholds[i-1] + thresholds[i - 2]) / 2

        return best_gini,best_idx,best_split_type,best_thr
